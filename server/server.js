...
if (!MAIL_FROM || !MAIL_TO) {
  console.error('環境変数 MAIL_FROM と MAIL_TO は必須です。.env を確認してください。');
  process.exit(1);
}
...
const limiter = rateLimit({
  ...
  legacyHeaders: false,
  message: { ok: false, error: '送信回数が多すぎます。しばらく待って再度お試しください。' },
});
...
app.post('/api/contact', limiter, async (req, res) => {
  try {
    ...
    if (!name || !isEmail(email) || !message) {
      return res
        .status(400)
        .json({ ok: false, error: 'お名前・有効なメールアドレス・お問い合わせ内容は必須です。' });
    }
